...
8. 宁可多检索也不要遗漏重要的相关记忆信息`,
  ...
  },
  {
    ...
    description: `💾 [自然记忆器] 每轮对话自动提取并存储语义鸿沟内容
AI的自然记忆行为：像呼吸一样自然地在每轮对话后记录新知识。

核心理念:
🎯 自然记忆：每轮对话必须记忆，无需判断"是否值得"
🔍 语义鸿沟：只记录AI预训练知识之外的新信息
✂️ 奥卡姆剃刀：用最简洁的话提取核心增量知识

什么是语义鸿沟？
- AI已知：通用知识、公开信息、训练数据中的内容 ❌不记
- AI未知：用户特定信息、项目细节、实时变化、个人偏好 ✅记忆
- 判断标准：这填补了我的认知空白吗？

记忆提取示例:
对话："帮我优化PromptX的记忆系统"
→ 记忆："PromptX项目：正在优化记忆系统为自然记忆+语义鸿沟提取"

对话："我更喜欢函数式编程，别用面向对象"
→ 记忆："用户编程偏好：函数式>面向对象"

对话："Python的print函数怎么用？"
→ 不记忆（AI已知的通用知识）

你必须:
1. 每轮对话结束时自动执行记忆（强制要求）
2. 识别语义鸿沟：只记录AI不知道的新信息
3. 奥卡姆剃刀：一句话总结本轮核心增量知识
4. 专注于：做了什么 + 学到什么 + 用户特征
