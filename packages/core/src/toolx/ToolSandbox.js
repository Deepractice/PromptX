...
const { ToolError, VALIDATION_ERRORS, SYSTEM_ERRORS, DEVELOPMENT_ERRORS } = require("./errors")
...
class ToolSandbox {
  ...
  async init() {
    ...
    try {
      ...
      // 初始化失败是系统错误
      throw new ToolError(`Failed to initialize ToolSandbox: ${error.message}`, SYSTEM_ERRORS.SANDBOX_INIT_FAILED.code, { originalError: error.message })
    }
  }
  ...
  async analyze() {
    ...
    try {
      ...
      if (!resourceResult.success) {
        throw new ToolError(`Failed to load tool: ${resourceResult.error?.message || "Unknown error"}`, SYSTEM_ERRORS.TOOL_NOT_FOUND.code, { toolId: this.toolId })
      }
    ...
    try {
      ...
      if (Object.keys(this.dependencies).length > 0) {
        ...
        try {
          ...
          this.logger.info(`[ToolSandbox] Dependency analysis: ` + `${Object.keys(analysis.preinstalled).length} preinstalled, ` + `${Object.keys(analysis.required).length} need installation`)
    ...
    try {
      ...
      switch (action) {
        ...
        case "search":
          // 搜索日志
          if (!options.keyword) {
            throw new ToolError("Search action requires keyword parameter", VALIDATION_ERRORS.MISSING_REQUIRED_PARAM.code, { param: "keyword" })
          }
          return {
            success: true,
            action: "search",
        ...
        case "timeRange":
          ...
          if (!options.startTime || !options.endTime) {
            throw new ToolError("Time range action requires startTime and endTime parameters", VALIDATION_ERRORS.MISSING_REQUIRED_PARAM.code, { params: ["startTime", "endTime"] })
          }
          return {
            success: true,
            action: "timeRange",
        ...
        default:
          throw new ToolError(`Unknown log query action: ${action}`, VALIDATION_ERRORS.INVALID_PARAM_VALUE?.code || "INVALID_PARAM", { param: "action", value: action })
      }
    } catch (error) {
      const enhancedError = ToolError.from(error, {
        phase: "queryLogs",
  ...
  async createExecutionSandbox() {
    ...
    this.sandboxContext.importx = async moduleName => {
      ...
      try {
        ...
        throw new ToolError(`Cannot load module '${moduleName}': ${error.message}`, DEVELOPMENT_ERRORS.UNDECLARED_DEPENDENCY.code, { module: moduleName, originalError: error.message })
      }
    }
  ...
  extractToolId(toolReference) {
    ...
    throw new ToolError(`Invalid tool reference format: ${toolReference}`, VALIDATION_ERRORS.INVALID_PARAM_FORMAT?.code || "INVALID_PARAM", { toolReference })
  }
  ...
  parseToolContent(content) {
    try {
      ...
      throw new ToolError(`Failed to parse tool content: ${error.message}`, DEVELOPMENT_ERRORS.TOOL_SYNTAX_ERROR.code, { originalError: error.message })
    }
  }
