...
export class PreinstalledDependenciesManager {
  ...
  private isBuiltinModule(moduleName: string): boolean {
    const builtins = ["fs", "path", "os", "crypto", "util", "stream", "http", "https", "url", "querystring", "child_process"]
  ...
  public analyzeDependencies(toolDependencies: Record<string, string>): DependencyAnalysis {
    ...
    logger.info(`[DependencyAnalysis] Preinstalled: ${Object.keys(result.preinstalled).length}, ` + `Required: ${Object.keys(result.required).length}`)
